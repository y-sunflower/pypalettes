...
def test_get_one_palette_errors():
    ...
    with pytest.raises(
        ValueError, match=r"^keep list must be the same length as the palette"
    ):
...
def test_get_palette_errors():
    ...
    with pytest.raises(
        TypeError, match=r"^keep_first_n must be a positive integer, not "
    ):
        _get_palette("Acadia", keep_first_n="invalid type")  # ty: ignore

    with pytest.raises(
        TypeError, match=r"^keep_last_n must be a positive integer, not "
    ):
    ...
    with pytest.raises(
        TypeError, match=r"`name` must be a string or a list of strings"
    ):
        _get_palette(17)  # ty: ignore

    with pytest.raises(
        ValueError,
        match="Cannot specify more than one of keep_first_n, keep_last_n, and keep arguments simultaneously.",
    ):
